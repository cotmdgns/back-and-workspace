 ...
 *		- 코드의 중복을 제거하여 프로그램의 생산성과 유지 보수에 크기 기여 
 *
 * * 상속의 특징
 * 		- 클래스 간의 상속은 단일 상속만 가능 (부모는 하나!)
 * 		- 명시되지 않아도 모든 클래스는 Object 클래스 상속
 * 		  -> Object 클래스의 메서들을 오버라이딩 하여 메서드 재정의 가능
 * 		- 부모 클래스의 생성자, 초기화 블록의 상속되지 않는다.
 * 		  -> 자식 클래스 생성 시에 부모 클래스 생성자가 먼저 실행
 * 		- 부모의 private 변수는 상속은 되지만 직접 접근은 불가능
 * 		- Getter / Setter 사용해서 간접 접근 가능
 * */

public class Notbook extends Product{
	...
	public Notbook(String brand, String pCode, String name, int price,String cpu) {
	...
	public String toString() {
		return super.toString() + "Notbook [cpu=" + cpu + "]";
	}
