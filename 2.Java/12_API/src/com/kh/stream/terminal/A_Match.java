 ...
 * - noneMatch : 모든 요소들이 매개 값으로 주어진 조건을 만족하는 않는지 조사
 * */
...
public class A_Match {
	...
	public void method2() {
		...
		boolean result = students.stream()
				.filter(student -> student.getGenderr().equals("남자"))
				.allMatch(student -> student.getAge() >= 20);
		System.out.println("나이가 20살 이상인 학생들이 모두 남자인지 확인 : "+ result);
		
		// 남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는지 확인
		result = students.stream()
			.filter(student -> student.getGenderr().equals("남자"))
			.anyMatch(student -> (student.getEnglish()+student.getMeth()) / 2 >= 70)
			;
		System.out.println("남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는지 확인 : "+ result);
	}
