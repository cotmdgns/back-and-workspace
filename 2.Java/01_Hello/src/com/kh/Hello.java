 ...
 * 	 서로 관련된 코드들을 그룹으로 나누어 별도의 클래서를 구성한다.
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	 ...
	 * - 해당 메서드 호출 시 메소드의 블랙({}) 에 해당하는 부분을 실행한다. 
	 */
	// 실행 메서드(main 메서드) : 자바 프로그램 샐힝 시 가장 먼저 호출
	 ...
	 * - 클래스 : 대문자로 시작, 여러 개의 단어가 있을 때는 앞자리마다 대문자로 표기
	 * - 메소드 : 소문자로 시작, 여러 개의 단어가 있을 대는 앞마자마다 대몬나로 펴기
	 * 
	 * 
	 * 
	 * 
