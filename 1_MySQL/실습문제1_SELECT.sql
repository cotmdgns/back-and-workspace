...
SELECT *
FROM film_list
WHERE category In('Sci-Fi','Family') and rating In ('PG')and title LIKE '%GO%'; 
...
WHERE fid <= 7
	AND fid NOT IN (4,6);
-- WHERE (fid BETWEEN 0 and 6) and fid not in ('4','6');
...
SELECT substr(address, instr(address,' ')+1) addaress , district ''
FROM address
WHERE district != ''
ORDER BY 2,1 DESC
LIMIT 0, 10;
