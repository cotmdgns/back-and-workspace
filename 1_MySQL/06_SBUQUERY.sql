...
-- 전 직원의 평균 급여보다 더 많은 급여를 받은 사원들의 사번, 사원명, 직급코드, 급여 조회
...
-- 전지연 사원이 속해있는 부서원들의 사번, 직원명, 전화번호, 직급명, 부서명, 입사입 조회
-- 단, 전지연 사원의 제외
...
/*
	2. 다중행 서브쿼리
    - 서브쿼리의 조회 결과 값의 개수가 여러 행 일때(여러행 한열)
...
GROUP BY dept_code; -- 2890000,3660000,2490000,3760000
					-- 3900000,2500000,80000000
                    
-- 2) 위의 값들을 받는 직원의 이름, 직급코드, 부서코드, 급여 조회 
SELECT emp_name, job_code, dept_code, salary
FROM employee
WHERE salary IN (2890000,3660000,2490000,3760000,3900000,2500000,80000000);
...
-- 직원의 대한 사번, 이름 , 부서코드, 구분(사수/사원) 조회
SELECT manager_id
FROM employee;
-- 1 사수찾기? 누군가의 사수인 사람들의 id가 팔요 
...
/*
    ...
    -- > OR
*/
-- 대리 직급임에도 과장 직급들의 최소 급여보다 많이 받은 직원의 사번, 이름, 직급, 급여 조회
SELECT * FROM employee;
SELECT * FROM job;
...
FROM employee
	JOIN job USING(job_code)
WHERE job_name = '과장'; -- 2200000, 2500000, 3760000 --> 최소 급여

-- 2) 대리 직급임에도 과장 직급들의 최소 급여보다 많이 받은 직원의 사번, 이름, 직급, 급여 조회
SELECT emp_id, emp_name, job_name, salary 
FROM employee
	JOIN job USING(job_code)
WHERE job_name = '대리'
...
/*
	3. 다중열 서브쿼리
    - 서브쿼리의 조회 결과값이 한 행이지만 컬럼이 여러개일 떄 (한 행 여러 열)
*/
...
*/
-- 각 직급별로 최소 급여를 받은 사원들의 사번, 이름, 직급코드, 급여 조회
-- 각 직급별로 최소 급여
...
-- 각 부서별 최대 급여을 받는 사원들의 사번, 이름, 부서코드, 급여 조회 (부서없음 명시)
...
SELECT emp_id, emp_name, ifnull(dept_code,'부서업음'), salary
FROM employee
WHERE (ifnull(dept_code,'부서업음'), salary) 
								IN (SELECT ifnull(dept_code,'부서업음'), max(salary)
									FROM employee
									GROUP BY dept_code);