/*
	JOIN
    - 두 개 이상의 테이블에서 테이블를 조회하고자 할 때 사용하는 구문
    - 조회 결과는 하나의 결과물(RESULT SET)으로 나옴
    - 관계형 데이터테이스는 최소환의 테이터로 각각의 테이블에 담고 있음
		(중복을 최소화하기 위해 최대한 쪼개서 관리)
		부서 데이터는 부서 테이블, 사원에 대한 데이터는 사원 테이블, 직급 테이블 등...
        
        만약 어떰 사원이 어떤 부서에 속해있는지 부서명과 같이 조회하고 싶다면?
        만약 어떤 사원이 어떤 직급인지 직급명과 같이 조회하하고 싶다면? 
        
        => 즉, 괸계 데이터베이스에서 SQL 문을 이용한 티에블 간에"관계"를 맺어
			원하는 데이토만 조회하는 방법
*/
...
/*
    ...
    SELECT 컬럼, 컬럼... 
    FROM 테이블1
    [INNER](생략가능) JOIN 테이블 ON(테이블1.컬럼명 = 테이블2.컬럼명);
    
    -FROM 절에서 기준이 되는 테이블을 기술
    -JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭 시킬 컬럼에 대한 조건을 기술(ON, USING)
    -연결에 사용하려는 컬럼명이 같은 경우 ON 구문 대신 USING(컬럼명) 구문 가능 
*/
-- 1) 연결할 두 컬럼명이 다른 경우
-- 사번, 사원명, 부서코드 , 부서명 조회
...
FROM employee,job
WHERE job_code = job_name; -- ambiguous : 애매한,모호한/에러 발생!
...
WHERE e.job_code = j.job_code
	AND job_name = '대리';
-- 2) ANSI 구문
SELECT emp_id, emp_name, job_name,job_code, salary
FROM employee
join job USING (job_code)
WHERE job_name = '대리';
...
SELECT dept_id, dept_code, dept_title, emp_name, bonus 
FROM employee
...
-- 3. 보너스를 받은 사원들의 사번, 사원명, 보너스 ,부서명 조회(employee, department)
SELECT emp_id, emp_name,dept_id, dept_code, bonus, dept_title
FROM department
...
/*
    ...
    - 두 테이블 간의 JOIN 시 일치하지 않는 행동 포함시켜서 조회가 가능하다
...
FROM employee
	JOIN department ON (dept_code = dept_id);
-- > 부서 배치가 안된 사원 2명에 대한 정보 조회X
-- > 부서에 배정된 사원이 없는 부서도 정보 조회X
...
-- 2) RIGHT JOIB: 두 테이블 중 오른쪽에 기술된 테이블을 기준으로 JOIN
SELECT emp_name, dept_title, salary,salary*12 연봉
FROM employee
...
SELECT 
    ...
    m.emp_id"사원사번",m.emp_name"사원명", m.dept_code"사원부서코드", m.manager_id"사수사번"
FROM employee e
LEFT JOIN employee m ON (m.emp_id = e.manager_id);
    
/*
	5.카테시안곱(CARTESIAN PRODUCT / 교차 조인(CROSS JOIN)
    -조인되는 모든 테이블의 각 행들이 서로서로 모두 매팽된 데이터가 검새된다. (곱집합)
    - 두 테이블의 행들이 모드 곱해진 행들의 조합의 출력 -> 방대한 데이터 출력 -> 과부하 위험
*/
...
-- 사번, 직급명, 직원명, 부서명, 근무지역, 급여 조회
SELECT emp_id, job_name, emp_name, dept_code, local_name,salary
FROM employee
JOIN department ON (dept_code = dept_id)
JOIN job USING (job_code)
...
JOIN department ON (dept_code = dept_id)
JOIN job USING(job_code)
WHERE emp_name LIKE '전%' and emp_no LIKE '7%' and substr(emp_no,8,1);

-- 3. 보너스를 받은 직원들의 직원명, 보너스, 연봉, 부서명 ,근무지역 조회

SELECT job_name, bonus, dept_title, local_name ,local_name
FROM department
JOIN employee ON (dept_code = dept_id)
JOIN location ON (local_code = location_id)
JOIN job USING(job_code)
...
-- 4. 한국과 일본에서 근무하는 지구언들의 직원명, 부서명, 근무지역, 근무국가 조회
SELECT emp_name, dept_code, local_name, national_name
FROM department
JOIN employee ON (dept_code = dept_id)
JOIN location ON (local_code = location_id)
JOIN national USING (national_code)
WHERE national_name IN ('한국','일본');
-- 5. 각 부서별 평근 급여를 조회하여 부서명, 평급 급여 조회
...
-- 9. 부서가 있는 직원들의 직원명, 직급명, 부서코드, 부서명 근무지역 조회
