...
/*
	...
	5 SLEECT		* | 조회하고자 하는 컬럼명 as 별칭 | 함수
    1 FROM		조회하고자 하는 테이블명
    2 WHERE		조건식 (연산자들 가지고 기술)
    3 GROUP BY	그룹 기준에 해당하는 컬럼명
	4 HAVING		조건식 (그룹 함수를 가지고 기술)
    6 ORDER BY	컬럼명 | 컬럼순번 | 별칭 [ASC | DESC];
*/
-- 부서별 평근 급여가 300만원 이상인 부서의 평균 급여 조회
SELECT 
	dept_code,
    avg(salary) 평균급여
FROM employee
...
SELECT job_code, sum(salary)합계
FROM employee
GROUP BY job_code
HAVING 합계 > 10000000;

-- 부서별 보너스를 받는 사원이 없는 부서만 조호;
...
/*
    ...
    - cube : 컬럼1을 가지고 중간집계도 내고, 컬럼2를 가지고도 중간집계를냄
    
    MySQL에서의 rollup
    컬럼1, 컬럼2 with rollup
    
    grouping : rollup이나 cude에 의해 산출된 값이 해당 컬럼의 집합의 산출물이면 0, 아니면 1
    -> 집계된 값인지, 아닌지 정도만 구분 
...
/*
    ...
    MINUS (차집합) : 선행 쿼리문의 결과값에서 후행 퀴리문의 결과값을 뺀 나머지 결과값만 추출(MySQL X)
		--> INTERSECT, MINUS : WHERW절에서 AND 연산자를 사용해서 처리 가능 
*/
-- 1. UNION 
...
WHERE dept_code = 'D5'
UNION
-- (2) 급여가 300만원 초과인 사원별의 사번, 사원명, 부서코드, 급여 조회
SELECT emp_id,emp_name,dept_code,salary
FROM employee
WHERE salary > 3000000;

-- 부서 코드가 D5이거나 급여가 300만원 초과인 사원별의 사번, 사원명, 부서코드, 급여 조회
-- 위 쿼리문 대시 WHERE 절에 OR 연산자를 사용해서 처리 가능
SELECT emp_id,emp_name,dept_code,salary
FROM employee
