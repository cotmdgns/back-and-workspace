/*
    ...
    - 객체(Object)/스미카(Schema)를 만들고(CREATE),
	  변경하고(ALTER), 삭제(DROP)하는 언어
	- 즉, 실제 데이터 값이 아닌 구조 자체를 정의하는 언어
    
    MySQL에서 객체(구조) : 테이블(Talbe), 뷰(View), 인데스(Index),
...
-- 테이블에 데이터를 추기시키는 구문 ( DML : INSERT )
-- INSERT INTO 테이블명 VALUES(값, 값, ...);
INSERT INTO member 
	VALUES(1, 'user01', 'pass01','채승훈',
    '남', '010-0000-0000', 'aaa@naver.com' , '2024-05-28');
INSERT INTO member 
	VALUES(2, 'user02', 'pass02','채승훈','남', null, null, now());
INSERT INTO member 
	VALUES(2, 'user02', 'pass02','채승훈','남', null, null, current_date());
...
INSERT INTO mem_unique 
	VALUES(1, 'user02','pass01','김진주','ㄴ',null,null,null);
-- >> 성별에 유요한 값이 아니어도 들어아고 있음!
...
-- >> check 제약조건으로 not_null 조건또한 위배되었음을 확인!
...
CREATE TABLE pro_like(
    ...
    like_date DATE,
    PRIMARY KEY (mem_no, product_name) -- 복합으로 사용하면 둘다 확인함
);
...
WHERE grade_code = 2; 
-- >> 지식 테이블(member)에 2라는 값을 사용하고 있기 때문에 삭제 X
...
/*
	자식테이블 새엉시 외래키 제약조건을 부여할 때 삭제옵션 지정 기능 
    - 삭제옵션 : 부모테이블의 테이터 삭제 시 그 테이터를 사용하고 있는
			   자식테이블의 값을 어떻게 처리할건지
               
	1. ON DELETE RESTTICTED (기본값)
		: 자식데이터로 쓰이는 부모데이터는 삭제가 아예 안되게끔 
	2. ON DELETE SET NULL
		: 부모데이터 삭제시 해당 데이터를 쓰고 있는 자식테이터의 값을 NULL로 처리
    3. ON DELETE CASCADE
		: 부모테이터 삭제시 해당 테이터를 쓰고 있는 자식데이터도 같이 삭제 
*/
...
-- mem_grade 테이블에서 grade_code가 2번 등급 삭제
DELETE FROM mem_grade
WHERE grade_code = 2;
...
/*
    ...
    AS 서브쿼리;
*/
-- kh 스키마에 있는 employee 테이블 복사하여 새로운 테이블 생성 ( 외부 스키마 불러올떄 앞에다가 스키마.테이블)
