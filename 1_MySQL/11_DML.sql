/*
    ...
    - 데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나,
      ...
      1) INSERT INTO 테이블명 VALUES (값, 값, ...);
		- 테이블에 모든 컬럼에 대한 값을 INSERT 하고자 할 떄 사용한다.
        - 컬럼 순번을 지켜서 VALUES값을 나열해야 한다.
...
-- emp_dept 테이블에 employee에서 부서 코드가 D1인 직원의 사번, 이름, 부서코드, 입사입 추가하고
...
/*
    ...
    - WHERE 절을 생략하먄 모든 행의 데이터를 변경된다. (MySQL 방지)
...
-- 아시아 지역에 근무하는 직원들의 보너스를 0.3으로 변경 (변경 사항 테이블의 emp_salary)
-- 쌤
SELECT emp_id
FROM kh.employee
	JOIN kh.department ON (dept_code = dept_id)
