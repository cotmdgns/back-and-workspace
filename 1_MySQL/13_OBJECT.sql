...
ON sakila.payment(amount); -- 비고윺 인덱스 생성
...
-- 만들어진 프로시저 확인
SHOW PROCEDURE status; -- (status) 상태 확인
...
/*
    ...
    BEGIN
		DECLARE 반환값 자료형
        결과에 대항하는 SQL문 등 계산식(result로 처리)
        RETURN 반환값
    END //
...
-- 함수 호출 (함수는 CALL 필요없음, 프로시저는 CALL 필요
...
/*
	...
	- 입출고에 대한 데이터가 기록(INSERT) 될 때마다
	  해당 상품에 대한 재고수량을 매전 수정(UPDATE) 해야 되는 경우
...
-- 데이터 3개 추가
INSERT INTO product(pname, brand, price) VALUE ('갤럭시24','삼성','1910000');
INSERT INTO product(pname, brand, price, stock) VALUE ('갤럭시Z 플립4','삼성','1780000',10);
INSERT INTO product(pname, brand, price, stock) VALUE ('아이폰15','애플','1250000',20);

-- 2. 상품 입/출고 상세 입력 테이블 생성 (prodetail)
/*
	컬럼 : dcode / INT / 기본키 / AUTO_INCREMENT
		  pcode / INT / 외래키
          pdate / DATE / 기본키 현재날짜
          amount / INT / NOT NULL
          status /CHAR(2) / 입고,촐고 둘중 하나
*/
