/*
	-데이터베이스 : 데이터(date) + 베이스(base)
    -DBMS : Datebase Management System의 약자, 데이터베이스 관리 시스템
    ...
    -관계형(Relation1) 데이터 베이스
    1. 가장 대표적인 데이터베이스 시스템
    2. 데이터 테이블 형태로 저장하고 여러 테이블을 조합하여 비즈니스 관계를 도출하는 구조
    ...
    -SQL 종류
	 ...
	 4. DDL(Data Definition Language) : 데이터 정의어
		- DBMS의 구조를 정의하거나 변경, 삭제하기 위해 사용하는 언어alter
...
/*
    ...
    - 테이블에서 테이블을 조회할 때 사용하는 SQP문
    - SELECT를 통해서 조회돈 결과를 RESULT SET이라고 한다.(즉, 조회돈 행들의 집합)
    - 조회하고자 하는 컬럼들은 반드시 FROM 절에 기술한 테이블에 존재하는 컬럼이어야 한다. 
    - 모든 컬럼을 조회할 경우 컬럼명 대신 * 기호 사용
*/

-- employee 테이블 전체 사원의 모든 컬럼(*) 정보 조회
SELECT *
FROM employee;
...
-- 5. employee 테이블의 입사일(hire_date), 직원명(emp_name), 급여 조회(salary) 조회
SELECT hire_date,emp_name,salary
FROM employee;

/*
	컬럼 산술 연산
    -SELECT 절에 컴렁명 입력 부분에 산술션산자를 사용해서 결과를 조회할 수 있다.
*/
-- employee 테이블에서 직원명(emp_name), 직원 연복(salary * 12) 조회
...
/*
	리터럴
    -SELECT 절에 리터럴을 사용하면 테이블에 존재하는 테이터처럼 조회가 가능
*/
...
-- employee 테이블 부서코드(dept-code)조회
SELECT DISTINCT dept_code
FROM employee;

-- emplyoee 테이블에 직급코드(job_code), 부서코드(dept_code)조회
...
/*
    ...
    - 조회하고자 하는 테이블로부터 특정 조건에 만족하는 테이블만 조회하고자 할 때 사용
    - 이때 WHERE 절에 조건식을 제시
    - 조건식에는 다양한 연산자 사용 가능
...
-- 4.재직중(ent_yn 컬럼값이 'N')인 사원들의 사변, 사원명, 입사일(hire_date) 조회
SELECT emp_id,emp_name,hire_date,ent_yn
FROM employee
WHERE ent_yn = 'N';

-- OR(또는), AND(그리고)
-- 부서코드가 'D6'이거나 급여가 300만원인 이상인 사원들의 사원명, 부서코드, 급여 조회
SELECT emp_name,dept_code,salary
FROM employee
WHERE dept_code = 'D6' or salary >= 3000000;

-- 급여가 350만원 이상 600만원 이하를 받은 사원들이 사원명, 사번,급여 조회
...
/*
    ...
    '%' : 0글자 이상
    비교대상컬럼 LIEK '문자%' => 비교대상컬럼값이 문자로 "시작"되는걸로 조회
    비교대상컬럼 LIKE '%문자' => 비교대상컬럼값이 문자로 "끝"나는걸 조회
    비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값이 문자가 "포함"되는걸로 조회 (키워드 검색)
...
-- 이름 중에 '하'가 포홤된 사원들의 사원명,주민번호 (emp_no), 전회번호(phone)조회
...
-- 전화번호의 3번째 자리가 1인 사원들의 사번(emp id) 사원명 전화번호 이메일(email) 조회
...
*/
-- 보너스를 받지 않는 사원(bouns 값이 null)들의
-- 사변(id) 이름 (name) 급여(salary) 보너스(bonus) 조회
...
*/
-- 부서코드가 D5,D6,D8인 부서원들의 이름 부서코드 급여
SELECT emp_name, dept_code, salary
FROM employee
...
-- 급연(salary)가 200만원 이상인 사원들의 모든 컬럼조회
SELECT *
FROM employee
...
-- 3. 연봉(보너스포함X)이 4000만원 이상이고 보너스를 받지 않는 사원들의
