...
-- location_id 컬럼의 자료형읜 VARCHAR(2),
-- lname 컬럼은 기본값을 미국으로 변경 (자료형 : VARCHAR(20))
...
/*
	...
	- 데이터 값이 기록되어 있어도 같이 삭제한다. ( 삭제된 컬럼 복구는 불가능 )
...
ALTER TABLE dept_copy DROP COLUMN lname; -- 테이블에는 최소 한개의 컬럼이 존재해야한다.(삭제불가은)

-- member_grade 테이블의 grade_code
