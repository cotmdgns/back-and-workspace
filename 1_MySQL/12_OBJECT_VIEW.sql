...
/*
	2. 뷰 컬럼에 별칭 부여
    - 서브쿼리의 SELECT절에 함수식이니 산술연산식이 기술되어 있을 경우 반드시 별칭 지정!
*/
-- 사원의 사번, 사원명, 직급명, 성별(남/여), 근무년수를 조회할수 수 있는
-- SELECT 문을 (vw_emp_job) 생성
...
as SELECT 
	...
	timestampdiff(year,hire_date,now()) 근무
FROM employee
...
/*
	4.DML 구문으로 VIEW 조각이 불가능한 경우
*/
-- 1) 뷰 정의에 포함되지 않는 컬럼을 조적하는 경우
CREATE OR REPLACE VIEW vw_job
AS SELECT job_code FROM job;

-- INSERT (에러)
INSERT INTO vw_job(job_code,job_naem)
VALUES('J8','인턴');
...
-- 2) 뷰에 포함하지 않는 컬럼 중에 베이스가 되는 컬럼이 NOT NULL 제약조건이 지정된 경우
CREATE OR REPLACE VIEW vw_job
...
AS SELECT emp_id 사번,emp_name 사원명,emp_no 주민번호,salary 금여,salary * 12 연봉
	FROM employee;
...
INSERT INTO vw_emp_sal(사번, 사원명, 주민번호, 금여)
VALUES (300,'홍길동', 000000-0000000,3000000); -- 확인
...
UPDATE vw_emp_sal
SET 금여 = 9000000
WHERE 사번 = 200; -- 산술연산과 무관하 컬럼은 데이터 변경 가능!
...
-- 부서별 급여의 합계, 평균을 조회한 SELECT 문을 vw_groupdeot 뷰 정의
SELECT * FROM employee;
SELECT * FROM department;
SELECT * FROM job;

CREATE OR REPLACE VIEW vw_groupdeot
AS SELECT dept_code, sum(salary) 합계, avg(salary) 평균
	FROM employee
	GROUP BY dept_code;

SELECT * FROM vw_groupdeot;

-- INSERT (에러)
INSERT INTO vw_groupdeot
VALUES ('D11',80,40);
...
-- DELETE (에러)
DELETE FROM vw_groupdeot
WHERE 합계 = 52100000;

-- 5. DISTINCT 구문이 포홤된 경우
...
-- 사원ㄷㄹ의 사번, 사원명, 주민번호, 부서명 조회한 SELECT 문을 vw_joinemp 뷰 정의
...
VALUES (500,'송준호', 970109-0000000,'총무부');

INSERT INTO vw_joinemp(emp_id,emp_name,emp_no)
VALUES (500,'송준호', 970109-0000000); -- 가능!
...
UPDATE vw_emp
SET salary = 2000000
WHERE emp_id = 202; -- 에러 발생 ( WITH CHECK OPTION )
