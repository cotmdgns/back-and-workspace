...
public class A_TryCatch {
	 ...
	 * 				RuntimeException - ArithmeticExcaption, IndexOutofBoundsException
	 * 
	 * try ~ catch
	 * 
	 * try{
	 ...
	 * } catch(에외클래스 예외변수명){
	 * 
	 * 	   // try에서 예외가 발생하면 실핼을 멈추고 catch로 들어와서 예외 처리 코드 실행
	 * 
	 * } finally{
	 * 
	 *	   // 예외가 밸상하거나 발생하지 않든 무조건 실행되는 코드들 
	...
	public static void main(String[] args) {
		...
		try {
			...
			// printStackTrace : 예외발생 당시의 호출스택(Cell Stack)에 있었던
			//					   메서드의 정보와 예외 메시지를 화면에 출력
//			e.printStackTrace();
			// getMessage : 발생한 예외클래스의 객체에 저정된 메시지를 얻을 수 있음
