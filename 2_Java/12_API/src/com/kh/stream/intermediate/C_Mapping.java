...
public class C_Mapping {
	...
	public void method1(){
		...
		double avg = students.stream()
				.mapToInt(student -> student.getMeth())
				.average().getAsDouble();
	...
	public void method2() {
		List<String> list = Arrays.asList("Java Lambda","Stream Filtering Sortde Mapping");
...
//			.flatMap(str -> Arrays.stream(str.split(" ")))
			.forEach(System.out::println); 	 			// 둘다 같은 방식
//			.forEach(str -> System.out.println(str));	// 둘다 같은 방식
	}
	
	/*
	 * asDoubleStream(), asLongStream(), boxed()
	 * - asDoubleStream : IntStream, LongStream을 DoubleStream으로 변환
	 * - asLongStream : InsStream을 LongStream으로 변환
